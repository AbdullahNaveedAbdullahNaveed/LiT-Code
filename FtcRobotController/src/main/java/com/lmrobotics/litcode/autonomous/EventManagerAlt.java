 ...
 */
public class EventManagerAlt
{
    private ConcurrentLinkedQueue<EventBlockAlt> HLQ;
     ...
     */
    public EventManagerAlt(HardwareMap hardwareMap, String config, boolean rawConfigData)
    {
        ...
        if (rawConfigData)
        {
            HLQ = HLQGeneratorAlt.makeHLQFromString(config);
        }
        // Create the HLQ when we were given the name of a config file
        else
        {
            HLQ = HLQGeneratorAlt.makeHLQFromFile(config);
        }
        SampleAutoOpMode.debugHook = "EM init EPSs";
